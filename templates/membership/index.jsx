...
import { Button, Link } from '@oacore/design/lib/elements'
...
const Card = ({ plan, onClick }) => (
  <article className={styles.plan} key={plan.title}>
    <div className={styles.planHeader}>
      <h5>{plan.title}</h5>
      {plan.caption.url ? (
        <Link className={styles.planHeaderLink} href={plan.caption.url}>
          {plan.caption.title}
        </Link>
      ) : (
    ...
    <div className={styles.planContent}>
      ...
      <div className={styles.planContentButton}>
        <Button
          ...
          onClick={() => onClick(plan.title.toLowerCase(), plan.action.url)}
        >
          {plan.action.title}
        </Button>
      </div>
...
const MembershipPageTemplate = observe(({ data }) => {
  const { membership } = useStore()
  const router = useRouter()

  const handleCardActionClick = (plan, url) => {
    membership.setActivePlan(plan)
    router.push(url)
  }
